...
class Loggers:
    ...
    def on_train_batch_end_one(self, model, ni, imgs, targets, paths, vals):
        """Logs training batch end events, plots images, and updates external loggers with batch-end data."""
        log_dict = dict(zip(self.keys[:3], vals))
        # Callback runs on train batch end
        # ni: number integrated batches (since train start)
        if self.plots:
            if ni < 3:
                ...
                plot_images(imgs, targets, paths, f)
                if ni == 0 and self.tb and not self.opt.sync_bn:
                    log_tensorboard_graph_one(self.tb, model, imgsz=(self.opt.imgsz, self.opt.imgsz))
            if ni == 10 and (self.wandb or self.clearml):
                files = sorted(self.save_dir.glob("train*.jpg"))
                if self.wandb:
                    self.wandb.log({"Mosaics": [wandb.Image(str(f), caption=f.name) for f in files if f.exists()]})
    ...
    def on_fit_epoch_end(self, vals, epoch, best_fitness, fi):
        ...
        if self.csv:
            ...
            with open(file, "a") as f:
                ...
                # f.write(s + ",".join(["%20.5g" % v for v in [epoch] + vals]).rstrip(",") + "\n")
...
class GenericLogger:
    ...
    def log_graph_one(self, model, imgsz=(640, 640)):
        """Logs model graph to all configured loggers with specified input image size."""
        if self.tb:
            log_tensorboard_graph_one(self.tb, model, imgsz)
...
def log_tensorboard_graph(tb, model, imgsz=(640, 640)):
    ...
    try:
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")  # suppress jit trace warning
            tb.add_graph(torch.jit.trace(de_parallel(model), (im,ir), strict=False), [])
    except Exception as e:
        LOGGER.warning(f"WARNING ⚠️ TensorBoard graph visualization failure {e}")
