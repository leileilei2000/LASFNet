...
def create_dataloader_one(
    ...
    with torch_distributed_zero_first(rank):  # init dataset *.cache only once if DDP
        dataset = LoadImagesAndLabels_one(
            path,
            imgsz,
            batch_size,
    ...
    return loader(
        ...
        collate_fn=LoadImagesAndLabels_one.collate_fn4 if quad else LoadImagesAndLabels_one.collate_fn,
...
class LoadImagesAndLabels_one(Dataset):
    ...
    def __init__(
        ...
        try:
            ...
            assert cache_rgb["hash"] == get_hash(self.label_files_rgb + self.im_files)  # identical hash
        except Exception:
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if self.augment:
                img,  labels = random_perspective_one(
                    img,
                    # ir,
                    labels,
                    degrees=hyp["degrees"],
    ...
    def load_mosaic(self, index):
        ...
        for i, index in enumerate(indices):
            ...
            if i == 0:  # top left
                ...
                # ir4 = np.full((s * 2, s * 2, img.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
        ...
        # Augment
        img4,  labels4, segments4 = copy_paste_one(img4,  labels4, segments4, p=self.hyp["copy_paste"])
        img4,  labels4 = random_perspective_one(
            img4,
            # ir4,
            labels4,
            segments4,
    ...
    def load_mosaic9(self, index):
        ...
        # Augment
        img9, labels9, segments9 = copy_paste_one(img9, labels9, segments9, p=self.hyp["copy_paste"])
    ...
    def get_json(self, save=False, verbose=False):
        ...
        for split in "train", "val", "test":
            if self.data.get(split) is None:
                ...
                continue
            dataset = LoadImagesAndLabels_one(self.data[split])  # load dataset
            x = np.array(
                [
