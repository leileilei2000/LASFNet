...
# def feature_visualization(x, module_type, stage, n=32, save_dir=Path('runs/detect/exp')): #原改进后
#     """
#     x:              Features to be visualized
#     module_type:    Module type
#     stage:          Module stage within model
...
def feature_visualization(x, module_type, stage, n=32, save_dir=Path('runs/detect/exp')):  #blue
    """
    x:              Features to be visualized
    module_type:    Module type
    stage:          Module stage within model
    ...
    if 'Detect' not in module_type:
        ...
        if height > 1 and width > 1:
            ...
            for i in range(n):
                block = blocks[i].squeeze().detach().numpy()
                block = 1 - (block - np.min(block)) / (np.max(block) - np.min(block))
