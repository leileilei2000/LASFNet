...
def random_perspective(
    im, ir, targets=(), segments=(), degrees=10, translate=0.1, scale=0.1, shear=10, perspective=0.0, border=(0, 0)
):
...
def copy_paste(im, ir, labels, segments, p=0.5):
    """
    ...
    n = len(segments)
    if p and n:
        h_rgb, w_rgb, c_rgb = im.shape  # height, width, channels
        h_ir, w_ir, c_ir = im.shape  # height, width, channels
        im_new = np.zeros(im.shape, np.uint8)
        ir_new = np.zeros(ir.shape, np.uint8)
        for j in random.sample(range(n), k=round(p * n)):
            l, s = labels[j], segments[j]
            box = w_rgb - l[3], l[2], w_rgb - l[1], l[4]
            ioa = bbox_ioa(box, labels[:, 1:5])  # intersection over area
            if (ioa < 0.30).all():  # allow 30% obscuration of existing labels
                labels = np.concatenate((labels, [[l[0], *box]]), 0)
                segments.append(np.concatenate((w_rgb - s[:, 0:1], s[:, 1:2]), 1))
                cv2.drawContours(im_new, [segments[j].astype(np.int32)], -1, (1, 1, 1), cv2.FILLED)
                cv2.drawContours(ir_new, [segments[j].astype(np.int32)], -1, (1, 1, 1), cv2.FILLED)

        result_rgb = cv2.flip(im, 1)  # augment segments (flip left-right)
        result_ir = cv2.flip(ir, 1)  # augment segments (flip left-right)
        i_rgb = cv2.flip(im_new, 1).astype(bool)
        im[i_rgb] = result_rgb[i_rgb]  # cv2.imwrite('debug.jpg', im)  # debug
        i_ir = cv2.flip(ir_new, 1).astype(bool)
        im[i_ir] = result_ir[i_ir]  # cv2.imwrite('debug.jpg', im)  # debug


    return im, ir, labels, segments

def copy_paste_one(im, labels, segments, p=0.5):
    """
    Applies Copy-Paste augmentation by flipping and merging segments and labels on an image.
