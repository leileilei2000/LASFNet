...
def create_dataloader_rgb_ir(path1, path2,  imgsz, batch_size, stride, single_cls=False, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
                             rank=-1, world_size=1, workers=8, image_weights=False, quad=False, prefix='', sampler=None,shuffle=False,seed=0,):
...
#                         f = ['{}{}'.format(os.path.splitext(x)[0], '_co.png') for x in f]
#                 else:
...
#         # Check cache grb
#         self.label_files_rgb = img2label_paths(self.im_files)  # labels rgb
        
#         self.ir_files = img2ir_paths(self.im_files)
#         # self.label_files_ir = img2label_paths(self.ir_files)  # labels ir

#         cache_path_rgb = (p if p.is_file() else Path(self.label_files_rgb[0]).parent).with_suffix(".cache")
#         try:
#             cache_rgb, exists_rgb = np.load(cache_path_rgb, allow_pickle=True).item(), True  # load dict
#             assert cache_rgb["version"] == self.cache_version  # matches current version
#             assert cache_rgb["hash"] == get_hash(self.label_files_rgb + self.im_files + self.ir_files)  # identical hash
#         except Exception:
#             cache_rgb, exists_rgb = self.cache_labels(self.im_files, self.label_files_rgb, cache_path_rgb, prefix), False  # run cache ops
        
#         # Check cache ir

#         # cache_path_ir = (p if p.is_file() else Path(self.label_files_ir[0]).parent).with_suffix(".cache")
#         # try:
...
#         #     assert cache_ir["hash"] == get_hash(self.label_files_ir + self.ir_files)  # identical hash
#         # except Exception:
#         #     cache_ir, exists_ir = self.cache_labels(self.ir_files, self.label_files_ir, cache_path_ir, prefix), False  # run cache ops
        
#         # Display cache 
#         nf, nm, ne, nc, n = cache_rgb.pop("results")  # found, missing, empty, corrupt, total
#         # nf_ir, nm_ir, ne_ir, nc_ir, n_ir = cache_ir.pop('results')
        
#         if exists_rgb and LOCAL_RANK in {-1, 0}:
#             d = f"Scanning {cache_path_rgb}... {nf} images, {nm + ne} backgrounds, {nc} corrupt"
#             tqdm(None, desc=prefix + d, total=n, initial=n, bar_format=TQDM_BAR_FORMAT)  # display cache results
#             if cache_rgb["msgs"]:
#                 LOGGER.info("\n".join(cache_rgb["msgs"]))  # display warnings
#         assert nf > 0 or not augment, f"{prefix}No labels found in {cache_path_rgb}, can not start training. {HELP_URL}"
        
#         # if exists_ir and LOCAL_RANK in {-1, 0}:
#         #     d = f"Scanning {cache_path_rgb}... {nf_ir} images, {nm_ir + ne_ir} backgrounds, {nc_ir} corrupt"
...
#         labels, shapes, self.segments = zip(*cache_rgb.values())
#         nl = len(np.concatenate(labels, 0))  # number of labels
#         assert nl > 0 or not augment, f"{prefix}All labels empty in {cache_path_rgb}, can not start training. {HELP_URL}"
#         self.labels = list(labels)
#         self.shapes = np.array(shapes)
#         self.im_files = list(cache_rgb.keys())  # update
#         self.label_files_rgb = img2label_paths(cache_rgb.keys())  # update
#         self.ir_files = img2ir_paths(cache_rgb.keys())
#         # self.label_files_ir = img2label_paths(cache_ir.keys())
...
#             self.label_files_rgb = [self.label_files_rgb[i] for i in include]
...
#             self.label_files_rgb = [self.label_files_rgb[i] for i in irect]
#             # self.label_files_ir = [self.label_files_ir[i] for i in irect]
...
#         self.ims = [None] * n
#         self.irs = [None] * n
#         self.npy_ir_files = [Path(f).with_suffix(".npy") for f in self.ir_files]
...
#                 img, ir, labels = mixup(img, ir, labels, *self.load_mosaic(random.choice(self.indices)))
...
#             ir, ratio, pad = letterbox(img, shape, auto=False, scaleup=self.augment)
...
#             if self.augment:
#                 img, ir, labels = random_perspective(
#                     img,
#                     ir,
#                     labels,
#                     degrees=hyp["degrees"],
...
#             # augment_hsv(ir, hgain=hyp["hsv_h"], sgain=hyp["hsv_s"], vgain=hyp["hsv_v"])
...
#         ir = ir[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, to 3x416x416
#         ir = np.ascontiguousarray(ir)

#         img_all = np.concatenate((img, ir), axis=0)


#         # return torch.from_numpy(img), torch.from_numpy(ir), labels_out, self.im_files[index], shapes
#         return torch.from_numpy(img_all), labels_out, self.im_files[index], shapes
...
#             self.npy_ir_files[i],
#         )
#         if ir is None:  # not cached in RAM
#             if fn.exists():  # load npy
#                 ir = np.load(fn)
#             else:  # read image
#                 ir = cv2.imread(f)  # BGR
#                 assert ir is not None, f"Image Not Found {f}"
#             h0, w0 = ir.shape[:2]  # orig hw
#             r = self.img_size / max(h0, w0)  # ratio
#             if r != 1:  # if sizes are not equal
#                 interp = cv2.INTER_LINEAR if (self.augment or r > 1) else cv2.INTER_AREA
#                 ir = cv2.resize(ir, (math.ceil(w0 * r), math.ceil(h0 * r)), interpolation=interp)
#             return ir , (h0, w0), ir.shape[:2]  # im, hw_original, hw_resized
#         return self.irs[i] , self.ir_hw0[i], self.ir_hw[i]  # im, hw_original, hw_resized
...
#     def cache_ir_images_to_disk(self, i):
#         """Saves an image to disk as an *.npy file for quicker loading, identified by index `i`."""
#         f = self.npy_ir_files[i]
#         if not f.exists():
#             np.save(f.as_posix(), cv2.imread(self.ir_files[i]))
...
#             ir, _, (h, w) = self.load_ir(index)

#             # place img in img4
#             if i == 0:  # top left
#                 img4 = np.full((s * 2, s * 2, img.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
#                 ir4 = np.full((s * 2, s * 2, ir.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
#                 x1a, y1a, x2a, y2a = max(xc - w, 0), max(yc - h, 0), xc, yc  # xmin, ymin, xmax, ymax (large image)
#                 x1b, y1b, x2b, y2b = w - (x2a - x1a), h - (y2a - y1a), w, h  # xmin, ymin, xmax, ymax (small image)
#             elif i == 1:  # top right
#                 x1a, y1a, x2a, y2a = xc, max(yc - h, 0), min(xc + w, s * 2), yc
...
#         # Augment
#         img4, ir4, labels4, segments4 = copy_paste(img4, ir4, labels4, segments4, p=self.hyp["copy_paste"])
#         img4, ir4, labels4 = random_perspective(
#             img4,
#             ir4,
#             labels4,
#             segments4,
...
#         im, ir, label, path, shapes = zip(*batch)  # transposed
#         n = len(shapes) // 4
#         im4, ir4, label4, path4, shapes4 = [], [], path[:n], shapes[:n]
...
#                     0
#                 ].type(im[i].type())
#                 ir1 = F.interpolate(im[i].unsqueeze(0).float(), scale_factor=2.0, mode="bilinear", align_corners=False)[
#                     0
#                 ].type(ir[i].type())
#                 lb = label[i]
#             else:
...
#         return torch.stack(im4, 0), torch.stack(ir4, 0), torch.cat(label4, 0), path4, shapes4
