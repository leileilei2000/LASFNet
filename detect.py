...
def run(
    ...
    (save_dir / "labels" if save_txt else Path(save_dir)).mkdir(parents=True, exist_ok=True)  # make dir
    ...
    for (path, im, im0s, vid_cap, s),(path_, im2, im0s_, vid_cap_, s_) in zip(dataset, dataset2):
        with dt[0]:
            im = torch.from_numpy(im).to(model.device)
            im2 = torch.from_numpy(im2).to(model.device)
            ...
            im2 = im2.half() if model.fp16 else im2.float()  # uint8 to fp16/32
            im2 /= 255  # 0 - 255 to 0.0 - 1.0
            if len(im2.shape) == 3:
                im2 = im2[None]  # expand for batch dim
            if model.xml and im2.shape[0] > 1:
                ims2 = torch.chunk(im2, im.shape[0], 0)

        # Inference
        with dt[1]:
            visualize = increment_path(Path(save_dir) / Path(path).stem, mkdir=True) if visualize else False
            if model.xml and im.shape[0] > 1:
                pred = None
                for image in ims:
                    if pred is None:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if webcam:  # batch_size >= 1
                ...
                p2, im0_, frame = path_, im0s_.copy(), getattr(dataset2, 'frame', 0)

            p = Path(p)  # to Path
            p2 = Path(p2)

            save_path = str(save_dir / p.name)  # im.jpg
            save_path2 = str(save_dir / p2.name)  # im.jpg
            
            txt_path = str(save_dir / "labels" / p.stem) + ("" if dataset.mode == "image" else f"_{frame}")  # im.txt
            txt_path2 = str(save_dir / "labels" / p2.stem) + ("" if dataset2.mode == "image" else f"_{frame}")  # im.txt

            s += "%gx%g " % im.shape[2:]  # print string

            gn = torch.tensor(im0.shape)[[1, 0, 1, 0]]  # normalization gain whwh
            gn2 = torch.tensor(im0_.shape)[[1, 0, 1, 0]]  # normalization gain whwh

            imc = im0.copy() if save_crop else im0  # for save_crop
            imc2 = im0_.copy() if save_crop else im0_  # for save_crop

            annotator = Annotator(im0, line_width=line_thickness, example=str(names))
            annotator2 = Annotator(im0_, line_width=line_thickness, example=str(names))
            if len(det):
...
def parse_opt():
    ...
    parser.add_argument("--save-csv", default=False,action="store_true", help="save results in CSV format")
