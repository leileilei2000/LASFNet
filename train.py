...
def train(hyp, opt, device, callbacks):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # ir_image = irs.to(device, non_blocking=True).float() / 255
            imgs_rgb = imgs[:, :3, :, :]
            imgs_ir = imgs[:, 3:, :, :]
            # imgs_rgb = imgs_rgb
            # irs =  imgs_ir
            ...
            if RANK in {-1, 0}:
                ...
                # Plot
                if plots and ni < 3:
                    f1 = save_dir / f'train_batch{ni}.jpg'  # filename
                    f2 = save_dir / f'train_batch{ni}_ir.jpg'  # filename
                    # Thread(target=plot_images, args=(imgs, irs, targets, paths, f), daemon=True).start()
                ...
                callbacks.run("on_train_batch_end", model, ni, imgs_rgb, targets, paths, list(mloss))
            ...
            if f.exists():
                ...
                if f is best:
                    ...
                    results, _, _ = validate.run(
                        ...
                        iou_thres=0.6, #if is_coco else 0.60,  # best pycocotools at iou 0.65
                        single_cls=single_cls,
                        dataloader=val_loader,
                        save_dir=save_dir,
                        # save_json=is_coco,
...
def parse_opt(known=False):
    ...
    parser.add_argument("--data", type=str, default= "data/LLVIP.yaml", help="dataset.yaml path") 
    parser.add_argument("--hyp", type=str, default=ROOT / "data/hyps/hyp.scratch-low-asf.yaml", help="hyperparameters path")
    parser.add_argument("--epochs", type=int, default=300, help="total training epochs")
    parser.add_argument("--batch-size", type=int, default=48, help="total batch size for all GPUs, -1 for autobatch")
    ...
    parser.add_argument("--device", default="1,3", help="cuda device, i.e. 0 or 0,1,2,3 or cpu")
    parser.add_argument("--multi-scale", action="store_true", help="vary img-size +/- 50%%")
    parser.add_argument("--single-cls", action="store_true", help="train multi-class data as single-class")
    parser.add_argument("--optimizer", type=str, choices=["SGD", "Adam", "AdamW"], default="SGD", help="optimizer")
    parser.add_argument("--sync-bn", action="store_true", help="use SyncBatchNorm, only available in DDP mode")
    parser.add_argument("--workers", type=int, default=8, help="max dataloader workers (per RANK in DDP mode)")
    parser.add_argument("--project", default=ROOT / "runs/xiaorong", help="save to project/name")
    parser.add_argument("--name", default="LLVIP-yolov5s_fusion_(att)", help="save to project/name")
